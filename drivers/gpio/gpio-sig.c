...
static int __init leuze_init(void)
{
	...
	/* Creating device */
	if((device_create(dev_class, NULL, dev, NULL, "leuze_device")) == NULL){
		printk(KERN_INFO "Cannot create the Device 1\n");
		goto r_device;
	}


    if((request_irq(irqNumber, (irq_handler_t) leuze_irq_handler, IRQF_TRIGGER_FALLING, "leuze_device", NULL))){
					  ...
					  struct pt_regs *regs)
{
	...
	if(t != NULL){
		rcu_read_unlock();
		if(send_sig_info(SIGLEUZE, &info, t) < 0){
